...
public class CapabilitiesDBCache
    ...
    public Set<String> getFeatures(String node) {
        ...
        if (node != null) {
            ...
            Cursor c = db.rawQuery(
                    "SELECT * FROM " + DatabaseContract.CapsFeatures.TABLE_NAME + " WHERE " +
                            DatabaseContract.CapsFeatures.FIELD_NODE + "=?", new String[]{node});
            try {
                while (c.moveToNext()) {
                    String f = c.getString(
                            c.getColumnIndex(DatabaseContract.CapsFeatures.FIELD_FEATURE));
                    result.add(f);
                }
            } finally {
                c.close();
    ...
    public Identity getIdentity(String node) {
        ...
        SQLiteDatabase db = this.helper.getReadableDatabase();
        Cursor c = db.rawQuery(
                "SELECT * FROM " + DatabaseContract.CapsIdentities.TABLE_NAME + " WHERE " +
                        DatabaseContract.CapsIdentities.FIELD_NODE + "=?", new String[]{node});
        try {
            if (c.moveToNext()) {
                String name = c
                        .getString(c.getColumnIndex(DatabaseContract.CapsIdentities.FIELD_NAME));
                String category = c.getString(
                        c.getColumnIndex(DatabaseContract.CapsIdentities.FIELD_CATEGORY));
                String type = c
                        .getString(c.getColumnIndex(DatabaseContract.CapsIdentities.FIELD_TYPE));
    ...
    public Set<String> getNodesWithFeature(String feature) {
        ...
        if (feature != null) {
            SQLiteDatabase db = this.helper.getReadableDatabase();
            Cursor c = db.rawQuery(
                    "SELECT distinct(" + DatabaseContract.CapsFeatures.FIELD_NODE + ") FROM " +
                            DatabaseContract.CapsFeatures.TABLE_NAME + " WHERE " +
                            DatabaseContract.CapsFeatures.FIELD_FEATURE + "=?",
                    new String[]{feature});
            try {
                while (c.moveToNext()) {
    ...
    public boolean isCached(String node) {
        ...
        SQLiteDatabase db = this.helper.getReadableDatabase();
        Cursor c = db.rawQuery(
                "SELECT COUNT(*) FROM " + DatabaseContract.CapsIdentities.TABLE_NAME + " WHERE " +
                        DatabaseContract.CapsIdentities.FIELD_NODE + "=?", new String[]{node});
        try {
            if (c.moveToNext()) {
                int r = c.getInt(0);
    ...
    @Override
    public void store(String node, String name, String category, String type,

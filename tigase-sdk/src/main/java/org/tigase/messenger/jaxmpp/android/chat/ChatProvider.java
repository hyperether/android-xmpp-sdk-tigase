...
public class ChatProvider {
            ...
            try {
                ...
                if (openChatCursor.moveToNext()) {
                    account = openChatCursor.getString(
                            openChatCursor
                                    .getColumnIndex(DatabaseContract.OpenChats.FIELD_ACCOUNT));
                    jid = openChatCursor.getString(
                            openChatCursor.getColumnIndex(DatabaseContract.OpenChats.FIELD_JID));
                }
                openChatCursor.close();
                Log.i("ChatProvider", "Room history for " + account + ", " + jid);

                deleted = db.delete(DatabaseContract.OpenChats.TABLE_NAME,
                        DatabaseContract.OpenChats.FIELD_ID + " = ?",
                        new String[]{String.valueOf(chatId)});
                Log.i("ChatProvider",
                        "Removed " + deleted + " open conversations (" + chatId + ")");
    ...
    public long createChat(SessionObject sessionObject, JID fromJid,
                           String threadId) throws JaxmppException {
        final SQLiteDatabase db = dbHelper.getWritableDatabase();

        final ContentValues values = new ContentValues();
        values.put(DatabaseContract.OpenChats.FIELD_ACCOUNT,
    ...
    public long createMuc(SessionObject sessionObject, JID fromJid, String nickname,
                          String password) {
        final SQLiteDatabase db = dbHelper.getWritableDatabase();
...
//		}
        Cursor c = db.query(DatabaseContract.OpenChats.TABLE_NAME, columns,
                ...
                new String[]{sessionObject.getUserBareJid().toString(), jid
                        .getBareJid().toString()}, null,
                null, null, null);
        return c;
    }
    ...
    public Cursor getChat(final SessionObject sessionObject, final int chatId) {
        ...
        Cursor c = db.query(DatabaseContract.OpenChats.TABLE_NAME, columns,
                DatabaseContract.OpenChats.FIELD_ACCOUNT + " = ? and " +
                        DatabaseContract.OpenChats.FIELD_ID + " = ?",
                new String[]{sessionObject.getUserBareJid().toString(), String.valueOf(chatId)},
                null, null,
                null, null);
        return c;
    }
    ...
    public List<Room> getRooms(SessionObject sessionObject,
        ...
        Cursor c = db.query(DatabaseContract.OpenChats.TABLE_NAME,
                ...
                DatabaseContract.OpenChats.FIELD_ACCOUNT + " = ? and " +
                        DatabaseContract.OpenChats.FIELD_TYPE + " = " +
                        DatabaseContract.OpenChats.TYPE_MUC,
                new String[]{sessionObject.getUserBareJid().toString()}, null, null, null, null);
        try {
            while (c.moveToNext()) {
                ...
                String nickname = c
                        .getString(c.getColumnIndex(DatabaseContract.OpenChats.FIELD_NICKNAME));
                String password = c
                        .getString(c.getColumnIndex(DatabaseContract.OpenChats.FIELD_PASSWORD));

                Room room = new Room(id, context, roomJid, nickname);
                room.setPassword(password);
                if (timestamp != null) {
//					room.setLastMessageDate(new Date(timestamp - 1000 * 30));
                    room.setLastMessageDate(new Date(timestamp + 1));
                }
                rooms.add(room);
            }
        } finally {
    ...
    public void updateRoomState(SessionObject sessionObject, BareJID room, int state) {
        ...
        Cursor c = db.query(DatabaseContract.OpenChats.TABLE_NAME,
                ...
                DatabaseContract.OpenChats.FIELD_ACCOUNT + " = ? and " +
                        ...
                        DatabaseContract.OpenChats.FIELD_JID + "=?",
                new String[]{sessionObject.getUserBareJid().toString(), room.toString()}, null,
                null, null);

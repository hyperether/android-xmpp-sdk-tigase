...
public abstract class CursorRecyclerViewAdapter<VH extends android.support.v7.widget.RecyclerView.ViewHolder>
    ...
    /**
     * Returns the query filter provider used for filtering. When the provider is null, no filtering
     * occurs.
     *
     ...
     * android.widget.FilterQueryProvider#runQuery(CharSequence)} method is invoked when filtering
     * is requested by a client of this adapter.
     *
    ...
    /**
     * This method will move the Cursor to the correct position and call {@link
     * #onBindViewHolderCursor(android.support.v7.widget.RecyclerView.ViewHolder,
     * android.database.Cursor)} .
     *
     ...
     * * android.database.Cursor)} , {@link #onBindViewHolder(android.support.v7.widget.RecyclerView.ViewHolder,
     * int)}
     *
    ...
    /**
     * Runs a query with the specified constraint. This query is requested by the filter attached to
     * this adapter.
     * <p/>
     * The query is provided by a {@link android.widget.FilterQueryProvider}. If no provider is
     * specified, the current cursor is not filtered and returned.
     * <p/>
     * After this method returns the resulting cursor is passed to {@link #changeCursor(Cursor)} and
     * the previous cursor is closed.
     * <p/>
     * This method is always executed on a background thread, not on the application's main thread
     * (or UI thread.)
     * <p/>
     * Contract: when constraint is null or empty, the original results, prior to any filtering,
     * must be returned.
     *

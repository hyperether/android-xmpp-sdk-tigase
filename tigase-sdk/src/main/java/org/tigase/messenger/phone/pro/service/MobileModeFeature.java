...
public class MobileModeFeature {
    ...
    public static final String MOBILE_OPTIMIZATIONS_QUEUE_TIMEOUT =
            Features.MOBILE_V1 + "#presence_queue_timeout";
    ...
    public static void updateSettings(Account account, JaxmppCore jaxmpp, Context context) {
        ...
        String valueStr = accountManager
                .getUserData(account, MobileModeFeature.MOBILE_OPTIMIZATIONS_ENABLED);
        ...
        sessionObject.setUserProperty(MobileModeFeature.MOBILE_OPTIMIZATIONS_ENABLED,
                mobileOptimizations);
    }
    ...
    protected void setMobileMode(final boolean enable) {
        ...
        if (enable) {
            ...
            getMobileModeHandler().post(new Runnable() {
                @Override
                public void run() {
                    try {
                        for (JaxmppCore jaxmpp : jaxmppService.getMultiJaxmpp().get()) {
    ...
    protected void setMobileMode(JaxmppCore jaxmpp, boolean enable) throws JaxmppException {
        if (jaxmpp.getSessionObject().getProperty(Connector.CONNECTOR_STAGE_KEY) ==
            ...
            if (Features.MOBILE_V1.equals(xmlns)) {
                Integer timeout = jaxmpp.getSessionObject()
                        .getProperty(MOBILE_OPTIMIZATIONS_QUEUE_TIMEOUT);
                if (timeout != null) {
    ...
    private Handler getMobileModeHandler() {
        if (mHandler == null) {
            HandlerThread handlerThread = new HandlerThread("mobile_mod_thread");
            handlerThread.start();

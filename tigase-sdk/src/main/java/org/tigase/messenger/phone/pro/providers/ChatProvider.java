...
public class ChatProvider extends ContentProvider {
    ...
    private final static Map<String, String> openChatsProjectionMap = new HashMap<String, String>() {
...

        {
            ...
            put(DatabaseContract.OpenChats.FIELD_ID,
                    "open_chats." + DatabaseContract.OpenChats.FIELD_ID + " as " +
                            DatabaseContract.OpenChats.FIELD_ID);
            put(DatabaseContract.OpenChats.FIELD_JID,
                    "open_chats." + DatabaseContract.OpenChats.FIELD_JID + " as " +
                            DatabaseContract.OpenChats.FIELD_JID);
            put(ChatProvider.FIELD_NAME,
                    "CASE WHEN recipient." + DatabaseContract.RosterItemsCache.FIELD_NAME +
                            " IS NULL THEN " +
                            " open_chats." + DatabaseContract.OpenChats.FIELD_JID +
                            " ELSE recipient." +
                            DatabaseContract.RosterItemsCache.FIELD_NAME + " END as " +
                            ChatProvider.FIELD_NAME);
            put(ChatProvider.FIELD_UNREAD_COUNT,
                    "(SELECT COUNT(" + DatabaseContract.ChatHistory.TABLE_NAME + "." +
                            DatabaseContract.ChatHistory.FIELD_ID + ") from " +
                            DatabaseContract.ChatHistory.TABLE_NAME +
                            " WHERE " + DatabaseContract.ChatHistory.FIELD_ACCOUNT +
                            " = open_chats." +
                            ...
                            DatabaseContract.ChatHistory.STATE_INCOMING_UNREAD + ") as " +
                            ChatProvider.FIELD_UNREAD_COUNT);
            put(DatabaseContract.OpenChats.FIELD_TYPE,
                    "open_chats." + DatabaseContract.OpenChats.FIELD_TYPE + " as " +
            ...
            put(ChatProvider.FIELD_LAST_MESSAGE,
                    "(SELECT " + DatabaseContract.ChatHistory.FIELD_BODY + " FROM " +
                            ...
                            DatabaseContract.ChatHistory.FIELD_JID + " = open_chats." +
                            DatabaseContract.OpenChats.FIELD_JID +
                            " ORDER BY " + DatabaseContract.ChatHistory.FIELD_TIMESTAMP +
                            " DESC LIMIT 1) as " +
    ...
    protected final static int URI_INDICATOR_OPENCHATS = 2;
    protected final static int URI_INDICATOR_OPENCHAT_BY_ID = 3;
    protected final static int URI_INDICATOR_OPENCHAT_BY_JID = 4;
    /**
     * Chat message by /account/destinationBareJID/id
     */
    protected static final int URI_INDICATOR_CHAT_ITEM = 6;
    protected static final int URI_INDICATOR_GROUP_CHAT_ITEM = 10;

    protected static final int URI_INDICATOR_UNSENT = 7;

    /**
     * List of chat messages by /account/destinationBareJID
     */
    protected static final int URI_INDICATOR_CHATS_ACCOUNT = 8;
    protected static final int URI_INDICATOR_CHATS_ALL = 11;
    protected static final int URI_INDICATOR_GROUP_CHATS_ACCOUNT = 9;

    public static void initProvider(String authority) {
        ...
        switch (sUriMatcher.match(uri)) {
            case URI_INDICATOR_GROUP_CHATS_ACCOUNT:
                ...
                if (mId != null && values.getAsInteger(DatabaseContract.ChatHistory.FIELD_STATE) ==
                    ...
                    v.put(DatabaseContract.ChatHistory.FIELD_STATE,
                            DatabaseContract.ChatHistory.STATE_OUT_DELIVERED);
                    db.update(DatabaseContract.ChatHistory.TABLE_NAME, v,
                    ...
                    Log.d("ChatProvider",
                            "Message '" + values.get(DatabaseContract.ChatHistory.FIELD_BODY) +
                                    "' already in db.");
                    return null;
                }
            case URI_INDICATOR_CHATS_ACCOUNT:
                ...
                SQLiteDatabase db = dbHelper.getWritableDatabase();
                long rowId = db.insert(DatabaseContract.ChatHistory.TABLE_NAME,
                        DatabaseContract.ChatHistory.FIELD_JID,
                        values);
                Log.d("ChatProvider",
                        "Inserted id=" + rowId + " message to " +
                                DatabaseContract.ChatHistory.TABLE_NAME);
                if (rowId > 0) {
    ...
    private Long isChatMessageAddedAlready(final ContentValues values) {
        ...
        final String nickname = values
                .getAsString(DatabaseContract.ChatHistory.FIELD_AUTHOR_NICKNAME);
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection,
        ...
        switch (sUriMatcher.match(uri)) {
            case URI_INDICATOR_OPENCHATS:
                ...
                break;
            case URI_INDICATOR_OPENCHAT_BY_ID:
                cursor = queryOpenChats(projection,
                        "open_chats." + DatabaseContract.OpenChats.FIELD_ID + "=?",
            ...
            case URI_INDICATOR_UNSENT: {
                ...
                cursor = qb
                        .query(dbHelper.getReadableDatabase(), projection, selection, selectionArgs,
                                null, null,
                                sortOrder);
                break;
            }
            ...
            case URI_INDICATOR_GROUP_CHAT_ITEM:
                ...
                cursor = dbHelper.getReadableDatabase()
                        .query(DatabaseContract.ChatHistory.TABLE_NAME, projection,
...
//						new String[]{jid, account}, null, null, sortOrder);
    ...
    private Cursor queryOpenChats(String[] projection, String selection, String[] selectionArgs,
                                  String sortOrder) {
        final SQLiteQueryBuilder qb = new SQLiteQueryBuilder();
        ...
        return qb.query(dbHelper.getReadableDatabase(), projection, selection, selectionArgs, null,
                null, sortOrder);
    }
